...
RUN mkdir -p /opt/eureka && \
    tar -zxvf eureka-*.tar.gz -C /opt/eureka --strip-components 1 && \
    rm eureka-*.tar.gz && \
    chmod 755 /opt/eureka/bin/*.sh && \
    chmod 755 /opt/eureka/conf/*.sh
...
+ FROM openjdk:1.8.1
  - 设置基础镜像
  - eureka的jdk运行环境为1.8，因此这里直接指定一个包含jdk1.8运行环境的基础镜像
...
+ RUN mkdir -p /opt/eureka && \
  ...
  - mkdir -p /opt/eureka：创建部署包解压目录
  - tar -zxvf eureka-*.tar.gz -C /opt/eureka --strip-components 1：解压部署包
  - rm eureka-*.tar.gz：删除部署包
  - chmod 755 /opt/eureka/bin/*.sh：修改文件权限
  - chmod 755 /opt/eureka/conf/*.sh：修改文件权限
+ EXPOSE 18585
  - 暴露容器端口
+ WORKDIR /opt/eureka
...
```plain
docker build -f Dockerfile -t eureka:0.6.x .
```

+ docker build：构建镜像
+ -f：指定配置文件来构建镜像，如 Dockerfile
+ -t：指定镜像标签，如 eureka:0.6.x
  - eureka：镜像名称
  - 1.0.0：版本号
...
```plain
docker tag eureka:0.6.x ccr.ccs.tencentyun.com/aolingdata/eureka:0.6.x
```

推送到私有仓库

```plain
docker push ccr.ccs.tencentyun.com/aolingdata/eureka:0.6.x
```
...
services: # 服务列表
  eureka: # 服务名称
    restart: always # 无论容器退出状态如何，总是重启容器
    image: ccr.ccs.tencentyun.com/aolingdata/eureka:0.6.x # 镜像
    container_name: eureka # 容器名称
    ports:
      - 服务端口:服务端口
    extra_hosts:
