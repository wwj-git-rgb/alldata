...
COPY data-compare-*.tar.gz /

RUN mkdir -p /opt/data-compare && \
    tar -zxvf data-compare-*.tar.gz -C /opt/data-compare --strip-components 1 && \
    rm data-compare-*.tar.gz && \
    chmod 755 /opt/data-compare/bin/*.sh && \
    chmod 755 /opt/data-compare/conf/*.sh

EXPOSE 18585

WORKDIR /opt/data-compare
...
+ FROM openjdk:1.8.1
  - 设置基础镜像
  - data-compare的jdk运行环境为1.8，因此这里直接指定一个包含jdk1.8运行环境的基础镜像
...
+ RUN mkdir -p /opt/data-compare && \
  ...
  - mkdir -p /opt/data-compare：创建部署包解压目录
  - tar -zxvf data-compare-*.tar.gz -C /opt/data-compare --strip-components 1：解压部署包
  - rm data-compare-*.tar.gz：删除部署包
  - chmod 755 /opt/data-compare/bin/*.sh：修改文件权限
  - chmod 755 /opt/data-compare/conf/*.sh：修改文件权限
+ EXPOSE 18585
  - 暴露容器端口
+ WORKDIR /opt/data-compare
...
```plain
docker build -f Dockerfile -t data-compare:0.6.x .
```

+ docker build：构建镜像
+ -f：指定配置文件来构建镜像，如 Dockerfile
+ -t：指定镜像标签，如 data-compare:0.6.x
  - data-compare：镜像名称
  - 1.0.0：版本号
...
services: # 服务列表
  data-compare: # 服务名称
    ...
    image: data-compare:0.6.x # 镜像
    container_name: data-compare # 容器名称
    ports:
      - 服务port:服务port # 宿主机端口以及容器端口
    extra_hosts:
      - "mysql_host_ip:43.138.156.44" # 数据库服务主机
      - "服务ip:服务ip" # data-compare 服务主机
```
...
```plain
docker tag data-compare:0.6.x ccr.ccs.tencentyun.com/aolingdata/data-compare:0.6.x
```

推送到私有仓库

```plain
docker push ccr.ccs.tencentyun.com/aolingdata/data-compare:0.6.x
```
...
services: # 服务列表
  data-compare: # 服务名称
    restart: always # 无论容器退出状态如何，总是重启容器
    image: ccr.ccs.tencentyun.com/aolingdata/data-compare:0.6.x # 镜像
    container_name: data-compare # 容器名称
    ports:
      - 服务端口:服务端口
    extra_hosts:
